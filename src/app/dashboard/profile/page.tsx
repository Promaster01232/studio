            ...
            <div className="relative z-10 space-y-8 sm:space-y-12">
                <header className="space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-2 px-2">
                        <h2 className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.3em] text-primary">Institutional Statutory Authority</h2>
                        <h1 className="text-2xl sm:text-5xl font-black font-headline tracking-tighter text-foreground uppercase leading-tight sm:leading-none">Certificate Of Clearance Upgrade</h1>
                ...
                <div className="space-y-6 sm:space-y-8 max-w-2xl mx-auto text-center px-2">
                    <p className="text-xs sm:text-base font-medium text-muted-foreground leading-relaxed">
                        This Account Is Fully Authorized To Perform All Statutory Operations. Every AI System And Analytical Module Is Synchronized And 100% Ready To Execute All Forensic Tasks With Absolute Precision.
                    </p>
                </div>
                ...
                <Button onClick={downloadCertificate} className="w-full sm:w-auto rounded-xl h-12 sm:h-14 px-6 sm:px-10 font-black uppercase tracking-widest text-[10px] sm:text-xs shadow-2xl shadow-primary/20 active:scale-95">
...
export default function ProfilePage() {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (auth.currentUser) {
            const userDocRef = doc(firestore, "users", auth.currentUser.uid);
            setDoc(userDocRef, { photoURL: dataUrl }, { merge: true }).catch(async (err) => {
                const permissionError = new FirestorePermissionError({
                    path: userDocRef.path,
                    operation: 'update',
                    requestResourceData: { photoURL: dataUrl },
                } satisfies SecurityRuleContext, err);
                errorEmitter.emit('permission-error', permissionError);
            });
  ...
  const handleSaveChanges = () => {
    ...
    setSaving(true);
    const upd = { firstName, lastName, mobileNumber };
    const userDocRef = doc(firestore, "users", auth.currentUser.uid);
    setDoc(userDocRef, upd, { merge: true })
      .then(() => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10 max-w-6xl mx-auto pb-20 px-4 text-left">
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start">
            <div className="w-full lg:w-[380px] space-y-6">
                <Card className="glass shadow-2xl rounded-[2.5rem] overflow-hidden border-primary/5">
                    <div className="bg-primary/5 p-8 flex flex-col items-center text-center">
                        <div className="relative group mb-6">
                            <Avatar className="h-32 w-32 border-4 border-background shadow-2xl rounded-[2rem]">
                            ...
                            <div className="absolute -bottom-2 -right-2 flex gap-2 z-20">
                                <button className="h-9 w-9 rounded-xl bg-white dark:bg-zinc-800 shadow-2xl flex items-center justify-center border border-primary/10 hover:bg-primary hover:text-white transition-all" onClick={() => fileInputRef.current?.click()}><Camera className="h-4 w-4" /></button>
                        ...
                        <h2 className="text-2xl font-black tracking-tighter">{firstName} {lastName}</h2>
                ...
                <Card className="border-destructive/10 bg-destructive/5 shadow-xl rounded-[2.5rem] overflow-hidden p-8">
            ...
            <div className="flex-1 w-full space-y-8">
                <Card className="glass shadow-2xl rounded-[2.5rem] border-primary/5 overflow-hidden">
                    <CardHeader className="p-10 bg-primary/5 border-b border-primary/10"><CardTitle className="text-3xl font-black tracking-tight leading-none uppercase text-left">Identity Profile</CardTitle></CardHeader>
                    <CardContent className="p-10 space-y-8 text-left">
                        ...
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Contact Access (Mobile)</Label>
                        ...
                        <div className="flex justify-end pt-2">
                            <Button onClick={handleSaveChanges} disabled={saving} className="h-14 px-12 font-black uppercase tracking-widest text-xs shadow-xl shadow-primary/20 rounded-2xl active:scale-95 transition-all">
                                {saving ? <Loader2 className="mr-3 h-5 w-5 animate-spin" /> : <Edit className="mr-3 h-4 w-4" />} Synchronize Registry
                            </Button>
                        </div>
                    </CardContent>
                </Card>
