...
export default function ManagementConsolePage() {
  ...
  const handleDeleteUser = async (user: UserRecord) => {
    ...
    if (!window.confirm(`Permanently delete records for ${user.firstName} ${user.lastName}?`)) return;
    ...
    <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 px-4 sm:px-6 lg:px-8 pb-12">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-primary/5 pb-8">
        <div className="space-y-1 text-left">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground font-medium">Platform enrollment oversight and management.</p>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:w-auto">
            <Button variant="outline" className="h-11 px-6 border-primary/10 rounded-xl font-bold bg-white">
                <Sparkles className="mr-2 h-4 w-4 text-primary" />
                <span className="text-[10px] uppercase tracking-wider">Identity Audit</span>
            </Button>
            <Button className="bg-primary text-white h-11 px-6 rounded-xl font-bold shadow-lg">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <Card className="border-primary/5 shadow-sm bg-amber-50/30 rounded-2xl">
              <CardHeader className="p-3 sm:p-4 pb-1">
                  <CardDescription className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-amber-600">Pending Verification</CardDescription>
          ...
          <CardHeader className="bg-muted/5 border-b border-primary/5 px-4 sm:px-6 py-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                      ...
                      <CardDescription className="text-[10px] sm:text-xs font-medium mt-1.5">Real-time inspection of all platform accounts.</CardDescription>
                  </div>
              ...
              <ScrollArea className="w-full">
                  <Table className="min-w-[800px]">
                      <TableHeader className="bg-muted/20">
                          <TableRow>
                              <TableHead className="font-black text-[10px] uppercase tracking-widest pl-6 h-12">Identity</TableHead>
                              <TableHead className="font-black text-[10px] uppercase tracking-widest text-center h-12">Role</TableHead>
                              <TableHead className="font-black text-[10px] uppercase tracking-widest text-center h-12">Verification Status</TableHead>
                              <TableHead className="font-black text-[10px] uppercase tracking-widest text-right pr-6 h-12">Controls</TableHead>
                          </TableRow>
                      </TableHeader>
                      <TableBody>
                          {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                              <TableRow key={user.uid} className={cn("hover:bg-muted/5 border-b border-primary/5", !user.emailVerified && !ADMIN_EMAILS.includes(user.email.toLowerCase()) && "bg-amber-50/10")}>
                                      ...
                                      {user.emailVerified ? (
                                          ...
                                          <div className="flex flex-col items-center gap-1.5">
                                              ...
                                              <Button 
                                                  size="sm" 
                                                  variant="ghost" 
                                                  className="h-6 px-3 text-[8px] font-black uppercase text-primary border border-primary/10 rounded-full"
                                                  onClick={() => handleVerifyUser(user)}
                                                  disabled={processingUid === user.uid}
                                              >
                                                  {processingUid === user.uid ? <Loader2 className="h-2.5 w-2.5 animate-spin" /> : "Audit"}
                                              </Button>
                                          </div>
                                      )}
                                  </TableCell>
                                      ...
                                      <div className="flex items-center justify-end gap-2">
                                          ...
                                          <DropdownMenu>
                                              ...
                                              <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl shadow-2xl border border-primary/5 bg-white">
                                                  <DropdownMenuItem className="rounded-lg font-bold text-xs h-10 px-3 cursor-pointer"><Mail className="mr-3 h-4 w-4" /> Message</DropdownMenuItem>
                                          ...
                                          <Button 
                                              className={cn(
                                                  "h-9 px-5 font-black text-[10px] uppercase rounded-lg shadow-sm",
                                                  user.isBlocked ? "bg-white border border-primary/10 text-primary" : "bg-red-500 text-white"
                                              )}
